...
class FeatherFuryBot(discord.Client):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            print("Synced global commands:", [cmd.name for cmd in synced_commands])
            for guild in self.guilds:
                print(f"Syncing commands for guild: {guild.name} (ID: {guild.id})")
                try:
                    guild_synced = await self.tree.sync(guild=guild)
                    print(f"Successfully synced commands for guild {guild.name}: {[cmd.name for cmd in guild_synced]}")
                except Exception as e:
                    print(f"Failed to sync commands for guild {guild.name}: {e}")
            await asyncio.sleep(5)
        except Exception as e:
            print(f"Failed to sync command tree (global): {e}")
        print("Available global commands after sync:", [command.name for command in self.tree.get_commands()])
    ...
    async def attack(self, interaction: discord.Interaction, opponent: discord.Member, battlefield: str = "Random"):
        ...
        if opponent.id == self.user.id:  # FeatherFury
            ...
            if not is_grand_master:
                ...
                await update_stats_embed(self, guild, user.id, interaction=interaction)
                await update_stats_embed(self, guild, opponent.id, interaction=interaction)
                return
        ...
        battle["active"] = False
        from core.events import update_stats_embed
        await update_stats_embed(self, guild, winner.user.id, interaction=interaction)
        await update_stats_embed(self, guild, loser.user.id, interaction=interaction)
    ...
    async def enter_race(self, interaction: discord.Interaction, name: str, color: str):
        print(f"Executing /enter_race for user {interaction.user.display_name} with name={name}, color={color}")
        await interaction.response.defer()
        guild = interaction.guild
        user = interaction.user
        ...
        try:
            ...
            try:
                ...
                return
            print(f"Creating race role {name} with color {hex_color}")
            role = await guild.create_role(name=name, color=discord.Color.from_str(hex_color))
            print(f"Assigning race role {name} to user {user.display_name}")
            ...
            print(f"Failed to create or assign race role {name}: {e}")
    ...
    async def stats(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        if not profile:
            ...
            return
        from core.events import update_stats_embed
        await update_stats_embed(self, interaction.guild, target.id, interaction=interaction)
